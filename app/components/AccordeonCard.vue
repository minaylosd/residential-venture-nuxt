<template>
    <section class="px-5 mx-auto lg:px-9 md:max-w-[1336px] w-full">
        <div class="pt-16 md:pt-25">
            <h2 class="mb-8 md:text-4xl text-3xl font-medium leading-none text-darkgray font-wide uppercase">
                Вопрос-ответ
            </h2>
            <div class="flex flex-col gap-5 bg-[#fafafa] p-6 rounded-[48px]">
                <div v-for="(item, index) in faqItems" :key="index" :class="[
                    'flex flex-col items-start justify-between w-full overflow-hidden border-divider',
